 ...
 * (`../LessonsManager.tsx`, kept as a thin re-export shim for the teacher
 * studio call site — see that file's header comment).
 */
...
/**
 * The 11 lesson/chapter actions (Task K2 — plan de cours complet: 6 lesson
 * ops from before + 5 new chapter/move ops), owner-scoped in
 * lib/teacher/studio-actions.ts — the studio (Task C3-T4 / K2) is the ONLY
 * injector since Stage 1 (the admin no longer authors course content; its
 * former, ownership-blind versions in lib/admin/content-actions.ts are
 * gone). Kept as ONE flat action bag (not split lesson/chapter bags) so
 * `/enseigner/studio/cours/[slug]/editer` only ever threads a single
 * `actions` prop through.
 */
export type LessonActions = {
   ...
   *  + TUS upload authorization for this lesson. The studio injects its own
   *  ownership-gated version (createMyVideoUploadAction) — see
