...
export function ImagesManager({
  ...
  return (
    <section className="rounded-xl border border-ink/12 bg-paper-light p-4">
      ...
      <div className="mt-3">
        <Field icon={IconPhoto} label={t('main')} hint={t('mainHint')} filled={main.trim() !== ''} id="field-main-image">
          <div className="flex items-center gap-2">
            ...
            <input value={main} onChange={(e) => setMain(e.target.value)} placeholder={t('urlPlaceholder')} aria-label={t('main')} className={inputCls} />
      ...
      {/* Secondary images */}
      <div className="mt-4">
        <Field icon={IconPhoto} label={`${t('secondary')} · ${secondary.length}`} hint={t('secondaryHint')} filled={secondary.length > 0}>
          {secondary.length > 0 && (
            <ul className="space-y-1.5">
              {secondary.map((img, i) => (
                <li key={img.id} className="flex items-center gap-2 rounded-lg border border-ink/10 bg-paper p-2">
                  ...
                  <button type="button" onClick={() => act(() => actions.moveSecondary(slug, img.id, 'up'))} disabled={i === 0} aria-label={t('moveUpAria')} title={t('moveUpAria')} className={iconBtn}><IconChevronUp size={12} /></button>
                  <button type="button" onClick={() => act(() => actions.moveSecondary(slug, img.id, 'down'))} disabled={i === secondary.length - 1} aria-label={t('moveDownAria')} title={t('moveDownAria')} className={iconBtn}><IconChevronDown size={12} /></button>
                  <button type="button" onClick={() => act(() => actions.removeSecondary(slug, img.id))} aria-label={t('removeAria')} title={t('removeAria')} className={cn(iconBtn, 'text-stampred')}><IconTrash size={12} /></button>
                </li>
              ))}
            </ul>
          )}
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder={t('urlPlaceholder')} aria-label={t('urlPlaceholder')} className={cn(inputCls, 'flex-1')} />
            <input value={alt} onChange={(e) => setAlt(e.target.value)} placeholder={t('alt')} aria-label={t('alt')} className={cn(inputCls, 'w-32')} />
            <button
              type="button"
