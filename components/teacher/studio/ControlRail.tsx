...
const STEP_GLYPH: Record<number, string> = { 1: '①', 2: '②', 3: '③', 4: '④' };
 ...
 *
 * Two renderings of the same list: an always-open `<aside>` on desktop
 * (`lg:` and up) and a native `<details>` (collapsed by default, no JS
 * required to toggle) on mobile — Task D1's own 360px requirement calls for
 * the rail to "collapse" there rather than push the actual form below the
 * fold.
 */
export function ControlRail({
      ...
      {grouped.map((group) => (
        <li key={group.key}>
          <button
            ...
            className={cn(
              'flex w-full items-baseline gap-1.5 rounded font-mono text-[11px] font-semibold uppercase tracking-wide',
              focusRing,
  ...
  return (
    <>
      ...
      {/* Mobile/360px — collapsible, no JS required to open (native <details>). */}
      <details className="rounded-xl border border-ink/12 bg-paper-light p-4 lg:hidden">
        <summary className={cn('flex cursor-pointer list-none items-center justify-between gap-2 font-mono text-[11px] uppercase tracking-wide', focusRing)}>
          <span className="text-ink/55">{t('checklist')}</span>
          <span className={missing === 0 ? 'text-teal' : 'text-ochre'}>
            {missing === 0 ? tr('complete') : tr('missingCount', { count: missing })}
          </span>
        </summary>
        <div className="mt-3">{list}</div>
