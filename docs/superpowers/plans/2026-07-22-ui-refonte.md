...
**U4 — Catalogue /formations : découverte complète.**
Add a `category` field to `data/courses.ts` (`'biznis' | 'dijital' | 'lajan' | 'lavi-pratik'` — assign each of the 9 sensibly; bilingual labels in messages). Page header as document (« KATALÒG · 9 FÒMASYON »). **Toolbar** (client component, URL-synced via searchParams): text search (matches title+tagline+learn bullets, both languages, debounced), category chips (Tout + 4), sort select (Prix ↑ / Prix ↓ / A–Z; « Pi popilè » and « Mye note » appear later with real data), results count (« 6 fòmasyon »), styled empty state (« Nou pa jwenn anyen pou "X" » + reset). Cards get the hover grammar; each card: code, seal, title, category tag, 3 learn-bullets, price USD+HTG, lesson count.
*Accept: search+filter+sort compose correctly and survive reload via URL params; both languages; empty state styled.*

**U4bis — Page prof /prof/[slug] + parcours /enseigner.**
NEW `data/teachers.ts`: teacher #1 (slug `pnice-academy`; display name, bio ht/fr, photo — reuse the founder photo/story content), his `courseSlugs` = all 9. NEW page `/prof/[slug]` per the spec's teacher-page vision, marketplace-ready but rendering static data today: document-style header (personal seal with initials, name, « Anseyan depi 2026 », note placeholder « — » until reviews exist), stats row (mono: 9 fòmasyon · X leson · 2 lang), bio, his course grid (reuse catalog cards), his subscription offer ($79 block), CTA. Course sales pages (U5) link their teacher block here. NEW page `/enseigner`: the become-a-teacher journey — hero (« Ou metrize yon konesans? »), 4-step numbered sequence (real sequence → numbering justified: Kandidati → Apwobasyon → Kreye kou → Touche 70%), the money facts (70% / fikse pri w / retrè MonCash), FAQ (5 questions incl. « Kijan kont mwen vin kont anseyan? » — same account, approval flow), interest CTA (same capture as U3 TeachTeaser), honest « Byento » framing: candidacies open soon, leave your interest now. Nav/footer « Anseye » links point here (replace U1's #anseye anchor).
*Accept: /prof/pnice-academy renders fully in both languages; /enseigner persuasive + honest about timing; unknown prof slug → 404.*
